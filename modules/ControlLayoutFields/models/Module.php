...
class ControlLayoutFields_Module_Model extends Vtiger_Module_Model {
    ...
    function getSettingLinks()
    {
        $settingsLinks[] = array(
            ...
            'linklabel' => 'Settings',
            'linkurl' => 'index.php?module=ControlLayoutFields&parent=Settings&view=ListAll&mode=listAll',
            'linkicon' => ''
        );
        ...
        $settingsLinks[] = array(
            'linktype' => 'MODULESETTING',
            'linklabel' => 'Uninstall',
            'linkurl' => 'index.php?module=ControlLayoutFields&parent=Settings&view=Uninstall',
            'linkicon' => ''
        );
        ...
        if(!empty($conditions)) {
            foreach($conditions as $p_index => $p_info) {
                foreach($p_info['columns'] as $index => $info) {
                    if(!($info['groupid'])) {
                        $firstGroup[] = array('columnname' => $info['columnname'], 'comparator' => $info['comparator'], 'value' => $info['value'],
                            'column_condition' => $info['column_condition'], 'valuetype' => $info['valuetype'], 'groupid' => $info['groupid']);
                    } else {
                        $secondGroup[] = array('columnname' => $info['columnname'], 'comparator' => $info['comparator'], 'value' => $info['value'],
                            'column_condition' => $info['column_condition'], 'valuetype' => $info['valuetype'], 'groupid' => $info['groupid']);
                    }
                }
            }
